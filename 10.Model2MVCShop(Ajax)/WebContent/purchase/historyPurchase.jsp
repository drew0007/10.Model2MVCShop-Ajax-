...
<script type="text/javascript">
	...
	$(function(){
		...
		$("td:contains('배송하기')").on("click", function(){
			var index = $("td:contains('배송하기')").index(this);
			var prodNo = $("input[name='prodNo']").val();
			var tranNo = $($("input[name='tranNoList']")[index]).val();
			var tranCnt = $($("input[name='tranCntList']")[index]).val();
			self.location="/purchase/updateTranCode?prodNo="+prodNo+"&tranNo="+tranNo+"&tranCnt="+tranCnt+"&tranCode=2";
		});
		
		$("td:contains('구매취소')").on("click", function(){
			var index = $("td:contains('구매취소')").index(this);
			var prodNo = $("input[name='prodNo']").val();
			var tranNo = $($("input[name='tranNoList']")[index]).val();
			var tranCnt = $($("input[name='tranCntList']")[index]).val();
			self.location="/purchase/updateTranCode?prodNo="+prodNo+"&tranNo="+tranNo+"&tranCnt="+tranCnt+"&tranCode=5";
		});		
	});
</script>
</head>
